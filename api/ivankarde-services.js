...
export default async function handler(req, res) {
  ...
  try {
    const apiId = parseInt(process.env.IRVANKARDE_API_ID) || 5522;
    ...
    if (!response.ok) {
      ...
      console.error('Irvan API HTTP error:', response.status, text);
    ...
    if (!data.status || !Array.isArray(data.services)) {
      console.error('Irvan response invalid:', data);
      return res.status(502).json({
        status: false,
